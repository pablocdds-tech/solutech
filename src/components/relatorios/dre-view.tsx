...
import { getReportDre } from "@/actions/relatorios";
import type { DreRow } from "@/actions/relatorios";
...
export function DreView() {
  const [data, setData] = React.useState<DreRow[]>([]);
  ...
  const loadData = React.useCallback(async () => {
    ...
    opts.dateTo = dateTo;
    const result = await getReportDre(opts);
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-slate-600">
        DRE derivado. Receitas de vendas, CMV de inventory_moves, despesas de AP pagos.
      </p>
      ...
      {!error && data.length > 0 && (
        <div className="overflow-x-auto rounded-lg border border-slate-200">
          <table className="w-full min-w-[400px] text-sm">
            ...
            <tbody>
              {data.map((row, i) => {
                const isResultado = row.category?.toLowerCase().includes("resultado") ?? false;
                const isPositive = row.amount >= 0;
                return (
                  <tr
                    key={i}
                    className={`border-b border-slate-100 ${
                      isResultado ? "bg-primary-50 font-bold" : ""
                    }`}
                  >
