-- =============================================
-- Vitaliano ERP — Migração 00016
-- FASE 11: Relatórios e Dashboard (M7/M8)
--
-- Views derivadas — relatórios NÃO armazenam resultados, SOMENTE calculam.
-- Filtros por loja e consolidado via parâmetros nas RPCs.
-- =============================================

-- ===========================================
-- 1. VIEW: v_dashboard_summary
-- KPIs consolidados por org/loja
-- ===========================================
...
COMMENT ON VIEW public.v_dashboard_summary IS
  'KPIs do dashboard por loja. Derivado, nunca armazenado.';

-- ===========================================
-- 2. RPC: fn_report_dre
-- DRE simplificado: Receitas - Custos - Despesas
-- ===========================================
...
BEGIN
  ...
  -- Receita bruta (vendas confirmadas)
  SELECT COALESCE(SUM(sa.total_amount), 0)
  INTO v_revenue
  FROM sales sa
  WHERE sa.org_id = v_org_id AND sa.status = 'confirmed'
    AND sa.sale_date >= p_date_from AND sa.sale_date <= p_date_to
    AND (p_store_id IS NULL OR sa.store_id = p_store_id);

  -- CMV (saídas de estoque por venda/produção)
  SELECT COALESCE(SUM(im.quantity * im.unit_cost), 0)
  INTO v_cmv
  FROM inventory_moves im
  WHERE im.org_id = v_org_id AND im.move_type = 'OUT'
  ...
  -- Despesas operacionais (AP pagos)
  SELECT COALESCE(SUM(ap.paid_amount), 0)
  INTO v_expenses
  FROM ap_payables ap
  WHERE ap.org_id = v_org_id AND ap.status = 'paid'
...
COMMENT ON FUNCTION public.fn_report_dre IS
  'DRE simplificado: receita - CMV - perdas - despesas. Derivado, nunca digitado.';

-- ===========================================
-- 3. RPC: fn_report_cash_flow
...
-- ===========================================
-- 4. RPC: fn_report_aging_ap
-- Aging de contas a pagar
-- ===========================================

CREATE OR REPLACE FUNCTION public.fn_report_aging_ap(
  p_store_id uuid DEFAULT NULL
)
RETURNS TABLE (
  aging_bucket text,
...
BEGIN
  ...
  SELECT
    ...
    END AS aging_bucket,
    COUNT(*) AS count,
    SUM(ap.amount - COALESCE(ap.paid_amount, 0)) AS total_amount
  FROM ap_payables ap
  ...
  ORDER BY
    CASE
      WHEN aging_bucket = 'A vencer' THEN 1
      WHEN aging_bucket = 'Vencido 1-30 dias' THEN 2
      WHEN aging_bucket = 'Vencido 31-60 dias' THEN 3
      WHEN aging_bucket = 'Vencido 61-90 dias' THEN 4
      ELSE 5
    END;
END;
$$;
...
-- ===========================================
-- 5. RPC: fn_report_aging_ar
-- Aging de contas a receber
-- ===========================================

CREATE OR REPLACE FUNCTION public.fn_report_aging_ar(
  p_store_id uuid DEFAULT NULL
)
RETURNS TABLE (
  aging_bucket text,
...
BEGIN
  ...
  SELECT
    ...
    END AS aging_bucket,
    COUNT(*) AS count,
    SUM(ar.amount - COALESCE(ar.received_amount, 0)) AS total_amount
...
-- ===========================================
-- 6. RPC: fn_report_stock_valuation
-- Valoração de estoque por loja
-- ===========================================
...
BEGIN
  ...
  SELECT
    ...
    ib.balance,
    ib.avg_cost,
    (ib.balance * ib.avg_cost) AS total_value
...
-- ===========================================
-- 7. RPC: fn_report_production_summary
...
BEGIN
  ...
  SELECT
    ...
    SUM(COALESCE(po.actual_quantity, 0)) AS total_actual,
    SUM(po.total_input_cost) AS total_input_cost,
    SUM(po.total_loss_cost) AS total_loss_cost,
    CASE WHEN SUM(COALESCE(po.actual_quantity, 0)) > 0
      THEN SUM(po.total_input_cost + po.total_loss_cost) / SUM(po.actual_quantity)
      ELSE 0 END AS avg_unit_cost
  FROM production_orders po
  WHERE po.org_id = v_org_id
    AND po.status = 'finalized'
    AND po.finalized_at >= p_date_from
    AND po.finalized_at < (p_date_to + interval '1 day')
...
-- ===========================================
-- 8. RPC: fn_report_virtual_bank_summary
-- Resumo do banco virtual por loja
-- ===========================================
...
BEGIN
  ...
  SELECT
    vl.store_id,
    SUM(CASE WHEN vl.entry_type = 'DEBIT' THEN vl.amount ELSE 0 END) AS total_debits,
    SUM(CASE WHEN vl.entry_type = 'CREDIT' THEN vl.amount ELSE 0 END) AS total_credits,
    SUM(CASE WHEN vl.entry_type = 'ADJUST' THEN vl.amount ELSE 0 END) AS total_adjusts,
    SUM(CASE
      WHEN vl.entry_type = 'DEBIT' THEN vl.amount
      WHEN vl.entry_type = 'CREDIT' THEN -vl.amount
      ELSE vl.amount
    END) AS balance
  FROM virtual_ledger_entries vl
  WHERE vl.org_id = v_org_id
  GROUP BY vl.store_id;
END;
$$;

-- ===========================================
-- 9. RPC: fn_report_checklist_ranking
-- Ranking de checklists por loja/score
-- ===========================================
...
BEGIN
  ...
  SELECT
    ...
    AVG(CASE WHEN ct.status = 'completed' THEN ct.score_pct END) AS avg_score,
    SUM(ct.nok_items) AS total_nok
